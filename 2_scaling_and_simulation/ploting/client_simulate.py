...
# python client_simulate.py --input_folder=simulated-data-2019-04-10 --north="41" --south="37" --east="-102" --west="-109" --start_datetime="2019-04-10 15:00:00" --end_datetime="2019-04-10 15:30:00" --output_filename="Colorado-2019-04-10-from-150000-to-153000" --keep_snapchat_images="0" --gif_duration=0.5
# python client_simulate.py --input_folder=simulated-data-2019-04-10 --north="41" --south="40" --east="-73" --west="-75" --start_datetime="2019-04-10 18:00:00" --end_datetime="2019-04-10 18:30:00" --output_filename="Newyork-2019-04-10-from-180000-to-183000" --keep_snapchat_images="0" --gif_duration=0.5
...
m = Basemap(llcrnrlon=(target_llcrnrlon-1),llcrnrlat=(target_llcrnrlat-1),urcrnrlon=(target_urcrnrlon+1),urcrnrlat=(target_urcrnrlat+1), lat_ts=0, resolution='h')
        ...
        if flight_lon >= target_llcrnrlon and flight_lon <= target_urcrnrlon \
            ...
            if (dest_lon <= target_urcrnrlon and dest_lon >= target_llcrnrlon and dest_lat <= target_urcrnrlat and dest_lat >= target_llcrnrlat) and \
                (origin_lon <= target_urcrnrlon and origin_lon >= target_llcrnrlon and origin_lat <= target_urcrnrlat and origin_lat >= target_llcrnrlat):
                    ...
                    plt.text(flight_lon+0.1, flight_lat, flight_tag, fontsize='smaller', color="#000000")
            ...
            if (dest_lon <= target_urcrnrlon and dest_lon >= target_llcrnrlon and dest_lat <= target_urcrnrlat and dest_lat >= target_llcrnrlat) and \
                (origin_lon >= target_urcrnrlon or origin_lon <= target_llcrnrlon or origin_lat >= target_urcrnrlat or origin_lat <= target_llcrnrlat):
                    ...
                    plt.text(flight_lon+0.1, flight_lat, flight_tag, fontsize='smaller', color="#006b1d")
            ...
            if (dest_lon >= target_urcrnrlon or dest_lon <= target_llcrnrlon or dest_lat >= target_urcrnrlat or dest_lat <= target_llcrnrlat) and \
                (origin_lon <= target_urcrnrlon and origin_lon >= target_llcrnrlon and origin_lat <= target_urcrnrlat and origin_lat >= target_llcrnrlat):
                    ...
                    plt.text(flight_lon+0.1, flight_lat, flight_tag, fontsize='smaller', color="#ff0000")

            # drawing passing routes - Blue
            if (dest_lon >= target_urcrnrlon or dest_lon <= target_llcrnrlon or dest_lat >= target_urcrnrlat or dest_lat <= target_llcrnrlat) and \
                (origin_lon >= target_urcrnrlon or origin_lon <= target_llcrnrlon or origin_lat >= target_urcrnrlat or origin_lat <= target_llcrnrlat):
                    m.scatter([flight_lon],[flight_lat],zorder=5,s=50,color="#0000FF",marker="o")
                    plt.text(flight_lon+0.1, flight_lat, flight_tag, fontsize='smaller', color="#0000FF")
