...
destination_airport_delay_df = df.withColumn("TOTAL_DELAY", col("WEATHER_DELAY")+col("CARRIER_DELAY")+col('NAS_DELAY')+col('SECURITY_DELAY')+col('LATE_AIRCRAFT_DELAY')).groupBy('DEST')

destination_airport_delay_df = destination_airport_delay_df.agg(
...
destination_airport_delay_df.show(showRowCount, False)

print("The total number of airports in destination is: " + str(destination_airport_delay_df.count()))
...
destination_airport_delay_df.repartition(1).write.csv('analyze_destination_airport_delay', header = 'true')
