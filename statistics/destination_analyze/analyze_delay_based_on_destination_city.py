...
destination_city_delay_df = df.withColumn("TOTAL_DELAY", col("WEATHER_DELAY")+col("CARRIER_DELAY")+col('NAS_DELAY')+col('SECURITY_DELAY')+col('LATE_AIRCRAFT_DELAY')).groupBy('DEST_CITY_NAME')

destination_city_delay_df = destination_city_delay_df.agg(
...
destination_city_delay_df.show(showRowCount, False)

print("The total number of citties in destination is: " + str(destination_city_delay_df.count()))
...
destination_city_delay_df.repartition(1).write.csv('analyze_destination_city_delay', header = 'true')
