...
state_route_delay_df = df.withColumn("TOTAL_DELAY", col("WEATHER_DELAY")+col("CARRIER_DELAY")+col('NAS_DELAY')+col('SECURITY_DELAY')+col('LATE_AIRCRAFT_DELAY'))

state_route_delay_df = state_route_delay_df.withColumn("ROUTE_BETWEEN_STATES", concat_ws("->", 'ORIGIN_STATE_NM', 'DEST_STATE_NM')).groupBy('ROUTE_BETWEEN_STATES') 

state_route_delay_df = state_route_delay_df.agg(
...
state_route_delay_df.show(showRowCount, False)

print("The total number of routes between 2 states is: " + str(state_route_delay_df.count()))
...
state_route_delay_df.repartition(1).write.csv('analyze_state_route_delay', header = 'true')
