...
origin_city_delay_df = df.withColumn("TOTAL_DELAY", col("WEATHER_DELAY")+col("CARRIER_DELAY")+col('NAS_DELAY')+col('SECURITY_DELAY')+col('LATE_AIRCRAFT_DELAY')).groupBy('ORIGIN_CITY_NAME')

origin_city_delay_df = origin_city_delay_df.agg(
...
origin_city_delay_df.show(showRowCount, False)

print("The total number of cities in origin is: " + str(origin_city_delay_df.count()))
...
origin_city_delay_df.repartition(1).write.csv('analyze_origin_city_delay', header = 'true')
